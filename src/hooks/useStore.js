...
const normalizeEvent = (event) => {
    ...
    return {
        ...
        posterPositionY: Number.isFinite(event.posterPositionY) ? Number(event.posterPositionY) : (Number.isFinite(event.coverPositionY) ? Number(event.coverPositionY) : 50),
        registrationUrl,
        weightTableGiUrl,
        weightTableNoGiUrl,
        circularUrl,
...
const useStoreState = (loadedState) => {
    ...
    const addEvent = async (event) => {
        ...
        const localEventDraft = {
            ...
            posterUrl: normalizeOptionalUrl(event?.posterUrl || event?.imageUrl || ''),
            posterPositionY: event?.posterPositionY != null ? Number(event.posterPositionY) : 50,
