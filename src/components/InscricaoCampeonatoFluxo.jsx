...
const InscricaoCampeonatoFluxo = ({ event, onComplete }) => {
  ...
  // Auto-selecionar o perfil do usuário e abrir direto na tela de Detalhes do Atleta / Cadastro (Etapa 1.5)
  useEffect(() => {
    ...
    if (!selectedProfile && selectableProfiles && selectableProfiles.length > 0) {
      setSelectedProfile(selectableProfiles[0]);
      if (stepParam === '2') {
        setStep(2);
      } else {
        setStep(1.5);
      }
    } else if (selectedProfile && step === 1) {
      if (stepParam === '2') {
        setStep(2);
      } else {
        setStep(1.5);
  ...
  }, [currentUser, selectableProfiles, selectedProfile]);
...
const ProfileConfirmationStep = ({ profile, event, onProfileUpdate, onConfirm, onBack }) => {
  ...
  const handlePhotoFileChange = async (e) => {
    ...
    try {
      ...
      if (typeof onProfileUpdate === 'function') {
        onProfileUpdate({ ...draftProfile, photoUrl: dataUrl });
      }
    } catch (err) {
      ...
      <div className="details-section profile-image">
        ...
        <div className="section-content centered">
          ...
          {!draftProfile.photoUrl && (
            <p className="photo-warning">
              <AlertCircle size={14} />
              Clique na bola acima para enviar a foto do atleta (obrigatoria para o evento).
            </p>
          )}
        </div>
      </div>
...
const CategorySelectionStep = ({ profile, event, registeredModalities = [], onConfirm, onBack }) => {
  ...
  const [weight, setWeight] = useState(() => {
    return profile?.weight || profile?.peso || '';
  });
  useEffect(() => {
    if (!weight && (profile?.weight || profile?.peso)) {
      setWeight(profile.weight || profile.peso);
    }
  }, [profile, weight]);
      ...
      <form onSubmit={handleSubmit} className="category-selection-form">
        <div className="form-group-pro">
          ...
          <div className="modality-grid-pro">
            ...
            {event?.noGiEnabled !== false && Number(event?.feeCombo || event?.priceCombo || 0) > 0 && (
              <button
                type="button"
        ...
        {event?.absoluteEnabled !== false && Number(event?.feeAbsolute || event?.priceAbsolute || 0) > 0 && (
