...
if (!code.includes("id=\"superFights\"")) {
    ...
    if (overviewPos !== -1) {
        ...
        const athletesPos = code.indexOf("{canManagePanel && activeSection === 'athletes' && (");
        if (athletesPos !== -1) {
             code = code.slice(0, athletesPos) + wrappedSf + code.slice(athletesPos);
        }
    }
}
...
if (regex.test(code)) {
    code = code.replace(regex, newPhotoLogic);
}
...
console.log('Successfully injected superFights and photo logic!');
