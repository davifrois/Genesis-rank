...
// The exact string for the modal end div
...
c = c.replace(modalEndStr, modalEndReplace);

// 3. Fix Map Preview
c = c.replace(
  /\{eventForm\.accommodationEnabled && \(\s*<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>.*?<\/div>\s*\)\}/s,
...
// 4. Add Icons
c = c.replace(
  /Youtube,\s*} from 'lucide-react';/,
  "Youtube,\n  Menu,\n  X,\n} from 'lucide-react';"
);

// 5. Add isMobileMenuOpen state
c = c.replace(
  /const \[eventSuccess, setEventSuccess\] = useState\(''\);\s*const \[showLogin, setShowLogin\] = useState\(false\);/,
  "const [eventSuccess, setEventSuccess] = useState('');\n  const [showLogin, setShowLogin] = useState(false);\n  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);"
);

// 6. Fix mainProfile logic
c = c.replace(
  /linkedProfiles\.forEach\(p => \{\s*let score = 0;\s*if \(\(p\.fullName \|\| ''\)\.toLowerCase\(\) === currentName\) score \+= 5;\s*if \(\(p\.email \|\| ''\)\.toLowerCase\(\) === username\) score \+= 3;\s*if \(score > bestScore\) \{\s*bestScore = score;\s*best = p;\s*\}\s*\}\);\s*return bestScore > 0 \? best : null;/,
...
// 7. Render Nav Item modification
c = c.replace(
...
// Add onClick to all Links in renderNavItem
c = c.replace(
  /to=\{entry\.path\}>/g,
  "to={entry.path} onClick={handleClick}>"
);
c = c.replace(
  /aria-current=\{isActive \? 'page' : undefined\}\s*>/,
  "aria-current={isActive ? 'page' : undefined}\n        onClick={handleClick}\n      >"
);

// 8. Replace topbar-main logic
const topbarMatch = /<div className="topbar-main">.*?<\/header>/s;
const topbarReplace = `<div className="topbar-main">
...
c = c.replace(topbarMatch, topbarReplace);
...
console.log('Fixed master script successfully!');
