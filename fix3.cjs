...
const heroEndMatch = content.match(/<\/Link>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/section>/);
if (!heroEndMatch) {
    console.error("Could not find hero end");
    process.exit(1);
}

const startIndex = heroEndMatch.index + heroEndMatch[0].length;

// Match the surviving news section Link
const substring = content.substring(startIndex);
const newsLinkMatch = substring.match(/<Link className="text-link" to="\/noticias">\{copy\.newsCta\}<\/Link>/);

if (!newsLinkMatch) {
    console.error("Could not find news link");
    process.exit(1);
}

const endIndex = startIndex + newsLinkMatch.index;
...
const newContent = content.substring(0, startIndex) + "\n" + missingBlock + content.substring(endIndex);
